...
**Goal:** validate the published macOS 12.3+ legacy Sender line, then move the
maintained work onto upstream `v3.4.2`.
...
**Current state:** legacy branch and `v3.3.0-intel-legacy.1` are published;
the Receiver remains unchanged and the build is ad-hoc signed.

**Start with:** run the physical Monterey acceptance checks, then create a clean
integration branch from upstream `v3.4.2` and port Sender changes in small groups.
